...
class DioConfig {
  ...
  static Dio createDioWithToken(String baseUrl, String token) {
    Dio dio = Dio(BaseOptions(
        connectTimeout: 30000, receiveTimeout: 30000, baseUrl: baseUrl));
    (dio.httpClientAdapter as DefaultHttpClientAdapter).onHttpClientCreate =
        (HttpClient client) {
      client.badCertificateCallback =
          (X509Certificate cert, String host, int port) => true;
  ...
  static requestInterceptor(
    ...
    String? token;
    if (isRequireAuth) {
      String demoToken = PreferenceUtils().getDemoToken() ?? '';
      String userToken = PreferenceUtils().getUserToken() ?? '';
      token = demoToken == '' ? userToken : demoToken;
      options.headers['Authorization'] = 'Bearer $token';
    }
    return handler.next(options);
  }
